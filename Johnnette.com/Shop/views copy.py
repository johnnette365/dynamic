...
def payment_done(request):
    ...
    for order_db in order_db:
        ...
        if payment['status'] == 'captured':
            OrderPlaced(order_id=order_db.order_id, user=user, address=address, razorpay_order_id=order_db.razorpay_order_id, product=order_db.product, quantity=order_db.quantity, total_amount=order_db.total_cost, payment_status=1, razorpay_payment_id=order_db.razorpay_payment_id, razorpay_signature=order_db.razorpay_signature).save()
            send_mail(
                f'Order Confirmation: {order_db.order_id} from JTPL STORE',
                
                f'Dear {address.full_name} \n\nThank you for placing an order with us. Your order has been received and is currently being processed. Here are the details of your order: \n\nOrder ID: {order_db.order_id}\nProduct: {order_db.product.product_title}\nQuantity: {order_db.quantity}\nTotal cost: {order_db.total_cost}\n\nWe will notify you via email once your order has been shipped. If you have any questions or concerns about your order, please do not hesitate to contact us at yk@johnnette.com or +919783642323.\n\nThank you for choosing our store.\n\nBest regards,\nJohnnette Technologies Pvt. Ltd.',
